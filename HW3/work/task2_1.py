...
N_CO_RATED_USERS = 3 # Minimum number of users that need to have co-rated both items
N_NEIGHBORS_ITEMS = 5 # number of neighbors for item-based collaborative filtering

def map_validation_items(
        item_hash: str, user_hash: str,
        index_items_train: dict,
        index_users_train: dict) -> tuple:
    '''Convert user and item hash into idx. If user or item not found return -1'''
    item_idx = index_items_train.get(item_hash) or -1
    user_idx = index_users_train.get(user_hash) or -1
...
if __name__ == '__main__':
    ...
    def pearson_similarity_w(item_1_idx: int, item_2_idx: int) -> float:
        ...
        item_2_mean_ratings = items_mean_ratings.value[item_2_idx]
        ...
        for user in similar_keys:
            ...
            cov_user_2 = user_ratings_2[user] - item_2_mean_ratings
            numerator += cov_user_1*cov_user_2

            # Denominator
            std_user_1 = (user_ratings_1[user] - item_1_mean_rating)**2
            std_user_2 = (user_ratings_2[user] - item_2_mean_ratings)**2
        ...
        if target_item == unkwon_token and target_user == unkwon_token: # new item and new user
            ...
            rating_pred = items_mean_ratings.value[target_item]
                ...
                for item, rating in items_rated_by_target_user.items():
                    key_pair = tuple(sorted([target_item, item]))
                    # sum(r_u_n*w_i_n)
                    if pearson_similarity_scores.value.get(key_pair):
                        w = pearson_similarity_scores.value[key_pair]
